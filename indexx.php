...
.navbar {
    ...
    width: 100%; /* Ajusta esto según qué tan ancho desees que sea el navbar */
    margin: 0 auto; /* Esto centrará el navbar en la página */
    border-bottom: 1px solid #d9d0d0;
}
...
.h2_de_carru { 
    ...
    margin: 0; /* Remove margin to ensure it fits well within its container */
...
.carousel-caption {
    display: flex; /* Use flex to center content both vertically and horizontally */
    align-items: center; /* Vertical centering */
    justify-content: center; /* Horizontal centering */
    width: 400px;   
    height: 100px;  
    margin: auto;
    overflow: hidden; /* In case the content somehow still overflows, hide it */
}
...
.card {
    max-width: 1600px; /* Ajusta según lo necesites */
}
...
.owl-carousel {
    position: relative;
    overflow: visible; /* Asegurarse de que los elementos que sobresalen sean visibles */
}
...
.owl-next {
    ...
    border: 1px solid #000; /* Borde delgado y negro */
    display: flex; /* Centrará el ícono en el botón */
    ...
    transform: translateY(-50%); /* Ajuste para un centrado vertical perfecto */
    z-index: 10; /* Asegurarse de que las flechas estén por encima de otros elementos */
}

.owl-prev {
    left: -45px; /* Mueve la flecha hacia afuera en el lado izquierdo */
}

.owl-next {
    right: -45px; /* Mueve la flecha hacia afuera en el lado derecho */
}
...
.owl-prev:hover, 
.owl-next:hover {
    background-color: rgba(255, 255, 255, 0.9); /* Fondo más opaco en hover */
    border-color: #333; /* Borde ligeramente más claro en hover */
}

.owl-prev:active, 
.owl-next:active {
    box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2); /* Sombra más corta cuando se presiona el botón */
}

/* Cambio de color en hover para los íconos */
.owl-prev:hover i, 
.owl-next:hover i {
    color: #555; /* Un gris oscuro para el hover del ícono */
}
...
<?php if (!isset($_SESSION['es_admin']) && !isset($_SESSION['id_usuario'])) { ?>
            <a href="index.php" class="btn btn-lg active" role="button" aria-pressed="true" id="registrar" data-bs-toggle="modal" data-bs-target="#login" style="background-color: #1E88E5;">
              <h6 class="h6_de_mierda"><n>LOG IN</n></h6>
            </a>
          <?php } ?>
